...
const proposalHubEntries = [
  ...
  },
  {
    ...
    items: [
      {
        title: "Ficha demo",
        description: "Ejemplo público de cómo queda una ficha digital terminada.",
        href: "/ficha-demo",
        type: "Demo pública",
      },
      {
...
function createProposalDraft() {
  ...
  return {
    ...
    acceptSubject: "",
    callLabel: "Solicitar llamada",
    callText: "",
    replyLabel: "Responder por email",
    replySubject: "",
    adjustLabel: "Pedir ajustes",
    adjustSubject: "",
...
function normalizeProposalRecord(rawProposal = {}) {
  ...
  proposal.acceptLabel = String(proposal.acceptLabel || "Aceptar propuesta");
  proposal.acceptSubject = String(proposal.acceptSubject || "");
  proposal.callLabel = String(proposal.callLabel || "Solicitar llamada");
  proposal.callText = String(proposal.callText || "");
  proposal.replyLabel = String(proposal.replyLabel || "Responder por email");
  proposal.replySubject = String(proposal.replySubject || "");
  proposal.adjustLabel = String(proposal.adjustLabel || "Pedir ajustes");
...
function decorateProposalLinePresetImages() {
  ...
  gallery.innerHTML = `
    <div class="proposal-preset-images__head">
      ...
      <label class="secondary-btn secondary-btn--compact proposal-upload-btn">
        Subir imágenes
        <input type="file" accept="image/*" multiple data-proposal-line-preset-upload style="position:absolute;width:0;height:0;opacity:0;overflow:hidden;pointer-events:none" />
      </label>
    </div>
    <div class="proposal-preset-images__grid" data-proposal-preset-drop-area>
      ...
      `).join("") : `<div class="dashboard-empty proposal-preset-drop-hint">Arrastra imágenes aquí o usa el botón de arriba.</div>`}
    </div>
  `;
  ...
  const dropArea = gallery.querySelector("[data-proposal-preset-drop-area]");
  if (dropArea) {
    ["dragenter", "dragover"].forEach((ev) =>
      dropArea.addEventListener(ev, (e) => { e.preventDefault(); dropArea.classList.add("is-drop-active"); })
    );
    ["dragleave", "drop"].forEach((ev) =>
      dropArea.addEventListener(ev, (e) => { e.preventDefault(); dropArea.classList.remove("is-drop-active"); })
    );
    dropArea.addEventListener("drop", async (e) => {
      const files = Array.from(e.dataTransfer?.files || []).filter((f) => f.type.startsWith("image/"));
      if (!files.length) return;
      const newImages = (await Promise.all(files.map((f) => compressImage(f, 1600, 0.84)))).filter(Boolean);
      proposalLinePresetDraft.exampleImages = [...(proposalLinePresetDraft.exampleImages || []), ...newImages];
      proposalLinePresetDraft.exampleImage = proposalLinePresetDraft.exampleImages[0] || "";
      proposalLinePresetDraft = normalizeProposalLinePresetRecord(proposalLinePresetDraft);
      renderProposalLinePresetsManager();
...
function renderPublicCatalog() {
  ...
  els.publicProjectsGrid.innerHTML = projects.length ? projects.map((project, index) => {
    ...
    const headline = String(project?.state?.headline || "Promocion inmobiliaria publicada").trim();
    return `
      <article class="public-project-card">
        ...
        <div class="public-project-card__body">
          ...
          <div class="public-project-card__footer">
            ...
            <a class="primary-btn primary-btn--compact" href="${escapeAttr(getProjectPublicUrl(project.id))}">Ver ficha completa</a>
          </div>
        </div>
      </article>
    `;
...
function buildProjectSlugSource(projectLike) {
  ...
  const projectName = String(source.projectName || projectLike?.name || source.headline || source.companyName || "").trim();
...
function getProjectPublicPath(projectLike, language = "") {
  ...
  const slug = sanitizeProjectSlug(projectState.publicSlug || project?.slug || buildProjectSlugSource(project));
  if (slug) {
...
function getProjectPublicUrl(projectId) {
  ...
  return new URL(getProjectPublicPath(projectId), url).toString();
}
